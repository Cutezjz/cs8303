...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        self.orientation = self.orientation + random.gauss(0,self.bearing_noise)
        #print (distance,steering_angle,length, tan(steering_angle))
        beta = (distance/self.length) * tan(steering_angle)
        
        if (beta > .001): #this means its turning
            R = distance/beta
            cx = self.x - (sin(self.orientation)*R)
            cy = self.y + (cos(self.orientation)*R)
            nx = cx + (sin(self.orientation+beta)*R)
            ny = cy - (cos(self.orientation+beta)*R)
            norientation = (self.orientation+beta) % (2*pi)
        else:
            nx = self.x + (distance*cos(self.orientation))
            ny = self.y +  (distance*sin(self.orientation))
            norientation = (self.orientation+beta)%2*pi
       
        result = robot(self.length)
        result.set(nx,ny,norientation)
